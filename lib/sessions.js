...
function createSession(userId, username, role) {
  ...
  sessions.set(token, { user: username, username, userId, role, created: Date.now(), expires: Date.now() + SESSION_MAX_AGE });
  return token;
}
...
function validateSession(token) {
  ...
  if (!session.username && session.user) session.username = session.user;
  return session;
}
