...
const lruOrder = []; // track access order for eviction
...
function get(key) {
  ...
  trackRoute(key, 'access');

  if (entry) {
    if (Date.now() > entry.expires) {
      hotCache.delete(key);
      removeLRU(key);
      logEvent('EXPIRE', key, 1, 0);
      stats.tier1.misses++;
      stats.total.misses++;
      trackRoute(key, 'miss');
      return null;
  ...
  stats.tier1.misses++;
  stats.total.misses++;
  logEvent('MISS', key, 1, 0);
  trackRoute(key, 'miss');
  return null;
}

function set(key, data, ttl, avgLatency) {
  ttl = ttl || getAdaptiveTTL(key);
  const size = estimateSize(data);

  // Evict if at capacity
  while (hotCache.size >= MAX_ENTRIES && lruOrder.length > 0) {
  ...
  touchLRU(key);
  stats.tier1.sets++;
  logEvent('SET', key, 1, 0);

  // Track route pattern for prefetch
  trackPrefetchPattern(key);
}
  ...
  return (req, res, next) => {
    ...
    res.json = (data) => {
      if (res.statusCode >= 200 && res.statusCode < 300 && data && !data.error) {
        const start = Date.now();
        set(key, data, baseTTL || getAdaptiveTTL(key));
      }
...
function invalidationMiddleware(req, res, next) {
  if (req.method === 'POST' || req.method === 'PATCH' || req.method === 'PUT' || req.method === 'DELETE') {
    // Invalidate matching GET caches
    const prefix = req.originalUrl.split('?')[0];
    const basePath = prefix.replace(/\/[^/]+$/, ''); // remove last segment
